  ...
  if info.is_player == 1:
    if info.content.startswith('!!sc'):
      if info.content.startswith('!!sc set changeTrue '):
        ...
        server.say('[SChanger]已将定时更换时间改为' + str(changeTime) + '秒,可能会滞后.')
      else:
        server.say(HelpMessage)
    if info.content.startswith('!!sc status'):
      server.say('[SChanger]定时更换时间为' + str(changeTime))
      server.say('[SChanger]自动换榜True的值为' + str(changeTrue))
...
def on_load(server, old):
  server.add_help_message('!!sc', '定时自动更换计分板')
  while True:
    change(server, old)
